...
class EvalDataset(Dataset[Dict[str, torch.Tensor]]):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        opened_image = Image.open(self.image_dir / image_path).convert("RGB")
        # Process Image --> get "pixel_values" (will either be a torch.Tensor OR a Dict[str,torch.Tensor])
        pixel_values = self.image_transform(opened_image)

        return dict(pixel_values=pixel_values, input_ids=input_ids, labels=labels, raw_text=caption, raw_image=opened_image)
...
class FinetuneDataset(Dataset[Dict[str, torch.Tensor]]):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for turn_idx, turn in enumerate(conversation):
            ...
            if isinstance(self.tokenizer, GPTNeoXTokenizerFast) or isinstance(self.tokenizer, LlamaTokenizer):
                pass
            else:
        ...
        if "image" in self.examples[idx]:
            ...
            # Set the <BOS> token's label to IGNORE_INDEX (since we're inserting the image patches right after)
            # Mamba/GPTNeoXTokenizer does not have the <BOS> token.
            if not isinstance(self.tokenizer, GPTNeoXTokenizerFast) and not isinstance(self.tokenizer, LlamaTokenizer):
                labels[0] = IGNORE_INDEX
...
class SarcasmDataset(Dataset[Dict[str, torch.Tensor]]):
    def __init__(
        self,
        instruct_json: Path,
        image_dir: Path,
