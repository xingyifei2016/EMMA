...
def decode(
    ...
    def get_logits(input_ids, inference_params):
        ...
        if not use_cache or not decoding:
            ...
            print("HERE")
        # print(logits.shape) # should be [1, 50288]
    ...
    if generate_end:
        # print("SHOULD STOP WILL NOT STOP ON END TOKEN with "+str(max_length))
        ...
        # print("SHOULD STOP WILL STOP ON END TOKEN!!!")
        def should_stop(current_token, inference_params):
            if inference_params.seqlen_offset == 0:
                return False
...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs
        ...
        if output_attn:
            output, attns = decode(
                input_ids, self, max_length=max_length, max_new_tokens=max_new_tokens, top_k=top_k, top_p=top_p, temperature=temperature, output_attn=output_attn, generate_end=generate_end, **kwargs
            )
            if not output_scores:
                output.scores = None
            outs = output if return_dict_in_generate else output.sequences
            return outs, attns

        else:
            output = decode(input_ids, self, max_length=max_length, max_new_tokens=max_new_tokens, top_k=top_k, top_p=top_p, temperature=temperature, generate_end=generate_end, **kwargs)
        if not output_scores:
            output.scores = None
    ...
    def forward(
        ...
        **kwargs
        ...
        if not self.fused_add_norm:
            ...
            # print("NOT FUSEDADDNORM: "+str(hidden_states.shape))
        else:
            ...
            # print("FUSEDADDNORM: "+str(hidden_states.shape))
...
class MambaForCausalLM(MambaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **backbone_kwargs
        ...
        outputs, intermediate_states= self.backbone(input_ids, inputs_embeds, intermediate=intermediate, inference_params=inference_params, 
                                               **backbone_kwargs)
