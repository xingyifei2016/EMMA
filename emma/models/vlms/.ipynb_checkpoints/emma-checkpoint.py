...
PyTorch Module defining a EMMA-VLM, our general interface for defining the various different VLMs in our work.
...
from emma.models.backbones.llm import LLMBackbone, MambaLLMBackbone
from emma.models.backbones.llm.prompting import PromptBuilder
from emma.models.backbones.vision import VisionBackbone
from emma.models.vlms.base_vlm import VLM
from emma.overwatch import initialize_overwatch
from emma.util.nn_utils import FusedMLPProjector, LinearProjector, MLPProjector, FusedLDPProjector
from emma.models.mamba.modeling_mamba import GenerationMixin as MambaGenerationMixin
...
import copy
from emma.models.mamba.modeling_mambav2 import create_block, _init_weights
...
class Mlp(nn.Module):
    # 2-layer MLP with activation in between 
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mamba_proj = create_block(dim,
            d_intermediate=0,
    ...
    def forward(self, q, kv, mask):
        ...
        q = q + self.mamba_proj(self.norm2(q))[0]
        return q
...
class EMMA_VLM(VLM):
    def __init__(
        ...
        if arch_specifier == "linear":
            ...
            raise ValueError(f"EMMA-VLM with `{arch_specifier = }` is not supported!")
        ...
        self.enc2dec = nn.Linear(img_embed_dim * 4, dec_embed_dim * 4)
        self.dec_block = nn.ModuleList([
            DecoderBlock(dec_embed_dim, dec_embed_dim // 64, 4, qkv_bias=True, qk_scale=None,
                         norm_layer=nn.LayerNorm)
            for i in range(4)])

        self.norm_1 = nn.LayerNorm(img_embed_dim)
        self.norm_2 = nn.LayerNorm(img_embed_dim)
        self.norm_3 = nn.LayerNorm(img_embed_dim)
        self.norm_4 = nn.LayerNorm(img_embed_dim)
        self.dec_norm = nn.LayerNorm(dec_embed_dim)
        self.dec_pred = nn.Linear(dec_embed_dim, 1152)
        self.base_dec = nn.Parameter(torch.zeros(1, 1, dec_embed_dim))
        self.dec_pos_embed = nn.Parameter(torch.zeros(1, 729, dec_embed_dim), requires_grad=False)
        self.register_buffer("mask", self.mask_generate(27, 27))
        self.dec_block.apply(self.atten_init_weights)
        trunc_normal_(self.base_dec, std=.02)
        self.patch_embed =  copy.deepcopy(self.vision_backbone.siglip_featurizer.patch_embed)
    ...
    def from_pretrained(
        ...
        """Initialize a EMMA-VLM from a pretrained checkpoint, freezing all weights, tailored for inference."""
        vlm = cls(
            model_id,
            vision_backbone,
            llm_backbone,
        ...
        ), "EMMA-VLM `from_pretrained` expects checkpoint with keys for `projector` AND `llm_backbone`!"
    ...
    def freeze_backbones(self, stage: str) -> None:
        ...
        if stage == "align":
            ...
            raise ValueError(f"Stage `{stage}` is not supported for LLaVa! Try < align | finetune | full-finetune >")
    ...
    def load_from_checkpoint(self, stage: str, run_dir: Path, pretrained_checkpoint: Optional[Path] = None) -> None:
        ...
        if self.arch_specifier.startswith("no-align"):
            overwatch.info(
                f"EMMA-VLM with `{self.arch_specifier = }` does not require pretrained weights!", ctx_level=1
            )
            return
    ...
    def get_fsdp_wrapping_policy(self) -> Callable:
        ...
        # Get EMMA-VLM Wrapping Policy =>> just a module wrapping policy around `self.projector`
        emma_fsdp_wrapping_policy = partial(
            _module_wrap_policy,
    ...
    def forward(
        ...
        # print(str(projected_patch_embeddings.shape)+","+str(input_embeddings[multimodal_indices, :, :].shape)+","+str(input_ids.shape))
        ...
        # Do image-level reconstruction
        img_emb = projected_patch_embeddings.shape[1]
        hidden_img = result.hidden_states[:, :img_emb, :] # last_img_feature
        features = result.intermediate

        features = [self.norm_1(features[0][:, :img_emb, :]), self.norm_2(features[1][:, :img_emb, :]),
                    self.norm_3(features[2][:, :img_emb, :]), self.norm_4(hidden_img)]
        features = self.enc2dec(torch.cat(features, dim=-1))

        B, N, C = features.shape
        features = features.reshape(B, 9, N//9, C)
        features = features.reshape(B, -1, C)
        B, N, C = features.shape
        features = features.reshape(B, N, C//4, 4)
        B, N, C, depth = features.shape
        
        base_dec = self.base_dec + self.dec_pos_embed
        H, W = int(np.sqrt(base_dec.shape[1])), int(np.sqrt(base_dec.shape[1]))
        ar_token = base_dec.reshape(base_dec.shape[0], H, W, C)
        ar_token = rearrange(ar_token, "b (h p1) (w p2) c -> b (h w) (p1 p2) c", p1=3, p2=3)
        ar_token = ar_token.reshape(1, -1, C)
        ar_token = ar_token.repeat(B,1,1)

        # apply Decoder blocks to decode the image
        count = 0
        for blk in self.dec_block:
            ar_token = blk(ar_token, features[:, :, :, count], self.mask)
            count += 1
            
        ar_token = self.dec_norm(ar_token)
        ar_token = self.dec_pred(ar_token)
        target = self.patch_embed(pixel_values['siglip'])
        ...
        target = rearrange(target, "b (h p1) (w p2) c -> b (h w) (p1 p2) c", p1=3, p2=3)
        target = target.reshape(B, -1, C)
        loss = (ar_token - target) ** 2
        try:
            result.loss += torch.sum(loss) * 1e-5
        except:
            print("Loss is not updated, should only happen when evaling!")
    ...
    def generate_eval(
        ...
        # Run LLM Forward --> returns CausalLMOutputWithPast!
        result = self.llm_backbone.generate(
            input_ids=None,
            attention_mask=None,
            position_ids=None,
            past_key_values=past_key_values,
            ...
            for idx, input_ids in enumerate(batch_input_ids):
                ...
                if return_string_probabilities is None:
                    ...
                    try:
                        ...
                        # print(str(token_probs)+" not except")
                    except:
                        token_probs = torch.softmax(full_out_dict.scores[0][0].float(), dim=0)
                        print(str(token_probs)+" except")
