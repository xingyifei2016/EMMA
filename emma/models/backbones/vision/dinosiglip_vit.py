...
from emma.models.backbones.vision.base_vision import ImageTransform, LetterboxPad, VisionBackbone, unpack_tuple
...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(self, vision_backbone_id: str, image_resize_strategy: str, default_image_size: int = 224) -> None:
        ...
        self.siglip_featurizer.embeds = unpack_tuple(self.siglip_featurizer.patch_embed)

        # self.siglip_featurizer.forward = partial(self.siglip_featurizer.get_intermediate_layers, n=len(self.siglip_featurizer.blocks))
        # self.dino_featurizer.forward = partial(self.dino_featurizer.get_intermediate_layers, n=len(self.dino_featurizer.blocks))
    ...
    def forward(self, pixel_values: Dict[str, torch.Tensor], mode="original") -> torch.Tensor:
        ...
        if mode == "siglip":
            dino_patches = self.dino_featurizer(pixel_values["dino"])
            siglip_patches = self.siglip_featurizer(pixel_values["siglip"])
            siglip_vals = self.siglip_featurizer.embeds(pixel_values["siglip"])
            return torch.cat([dino_patches, siglip_patches], dim=2), siglip_vals  

        elif mode == "tokenPacker":
            intermediate_blocks_dino = self.dino_featurizer(pixel_values["siglip"]) # should be length 24
            intermediate_blocks_siglip = self.siglip_featurizer(pixel_values["siglip"]) # should be length 27
            # old layers=[12,16,22,23] 
            layers=[12,16,19,20] 
            image_feature_list = []
            for l in layers:
                dino_ = intermediate_blocks_dino[l]
                siglip_ = intermediate_blocks_siglip[l]
                image_feature_list.append(torch.cat([dino_, siglip_], dim=2))
            image_features_multi = torch.cat(image_feature_list, dim=2)
            dino_fts = intermediate_blocks_dino[-2]
            siglip_fts = intermediate_blocks_siglip[-2]
            image_features = torch.cat([dino_fts, siglip_fts], dim=2)
