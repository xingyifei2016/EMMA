...
Pretraining script for EMMA VLM pretraining in native PyTorch, using Fully-Sharded Data Parallel (FSDP) to run
...
from emma.conf import DatasetConfig, DatasetRegistry, ModelConfig, ModelRegistry
from emma.models import get_llm_backbone_and_tokenizer, get_vision_backbone_and_transform, get_vlm
from emma.overwatch import initialize_overwatch
from emma.preprocessing import get_dataset_and_collator
from emma.training import Metrics, get_train_strategy
from emma.util import set_global_seed
...
class PretrainConfig:
    ...
    # ModelConfig (`emma/conf/models.py`); override with --model.type `ModelRegistry.<MODEL>.model_id`
    model: ModelConfig = field(
        default_factory=ModelConfig.get_choice_class(ModelRegistry.EMMA_3B.model_id)
    )

    # DatasetConfig (`emma/conf/datasets.py`); override with --dataset.type `DatasetRegistry.<DATASET>.dataset_id`
    ...
    hf_token: Union[str, Path] = Path("/home/lanxy/mamba_related/hf_token")                  # Environment variable or Path to HF Token

    # Tracking Parameters
    trackers: Tuple[str, ...] = ("jsonl", "wandb")                  # Trackers to initialize (if W&B, add config!)
    wandb_project: str = "emma"                                # Name of W&B project (default: `emma`)
...
def pretrain(cfg: PretrainConfig) -> None:
    overwatch.info("EMMA VLM Training :: Gathering Light")
    ...
    total_params = sum(p.numel() for p in vision_backbone.parameters())
    print("Vision total params is :"+str(total_params))
    ...
    overwatch.info(f"Instantiating EMMA-VLM `{model_id}` for Training Stage = `{cfg.stage}`")
    vlm = get_vlm(
        model_id,
